...
uses
  ...
  procedure _ExtractGameData2017EastAsia(sSource, sUrl: String);
  ...
  try
    ...
    repeat
      ...
      begin
        ...
        mlHome[iCnt].sName:= ReplaceText(RemoveHTMLTags(sl[3]), '#', '＃');
        mlHome[iCnt].sCard := '0';
        mlAway[iCnt].sPos := RemoveHTMLTags(sl[4]);
        mlAway[iCnt].sNum := RemoveHTMLTags(sl[5]);
        mlAway[iCnt].sName:= ReplaceText(RemoveHTMLTags(sl[6]), '#', '＃');
        mlAway[iCnt].sCard := '0';
        iCnt := iCnt + 1;
      end;
...
var
  sl : TStringList;
  iPos : Integer;
  sTmp, sTmp2, sErr : String;
begin
  sl := TStringList.Create;
  try
    try
      //大会名
      sTmp := CopyStr(sSource, '<a href="/match/EAFF_E1_Football_Championship_2017/">', '</a>');
      ...
      sGMD[3] := RemoveHTMLTags(sTmp);
      //Away
      sTmp := CopyStrNext(sSource, '<span class="team-name">', '</span>');
      sGMD[4] := RemoveHTMLTags(sTmp);
      ...
      if IsDebugMode then
        slList.SaveToFile('C:\Users\ragu\Desktop\Debug.txt', TEncoding.Unicode);
    except
      sErr := '下記の試合を解析中にエラーが発生しました。' + #13#10 +
              '対戦カード:' + sGMD[3] + ' x ' + sGMD[4] + #13#10 +
              '試合日:' + sGMD[1];
      ShowMessage(sErr);
    end;
  finally
