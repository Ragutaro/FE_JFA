...
uses
  ...
  procedure _ExtractGameData2015EmperorsCup(sSource, sUrl: String);
...
begin
  ...
  try
    try
      //大会名
      sTmp := CopyStr(sSource, '<a href="/match/emperorscup_2015/">', '</a>');
      ...
      if Not ContainsText(sGMD[3], '代表') then
        sGMD[3] := RemoveStringFromAtoB(sgmd[3], '（', '）');
      //Away
      sTmp := CopyStrNext(sSource, '<span class="team-name">', '</span>');
      sTmp := RemoveHTMLTags(sTmp);
      sTmp2 := ExtractStringsFromAtoB(sTmp, '（', '）');
      sGMD[4] := RemoveLeft(CopyStrToEnd(sTmp, '　'), 1) + sTmp2;
      if Not ContainsText(sGMD[4], '代表') then
        sGMD[4] := RemoveStringFromAtoB(sgmd[4], '（', '）');
